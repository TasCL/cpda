...
# !!! TO DO: types other than norm
{
  ...
  for ( i in 1:length(use.cells) ) {
    ...
    if (p.df$b[1] < p.df$A[1] | any(p.df$mean_v < 0) | any(p.df$t0 < 0) ) {
      ## cat("Catch b < A\n")
      for (j in 1:length(cells.to.fill)) if ( length(rts[[j]]) > 0 ) {
        likelihood[ attr(data,"cell.index")[[ cells.to.fill[j]]] ] <- 0
      }
    } else {
       # samp <<- try(
       #   rlba.norm(attr(data, "n.pda"),
       ...
       # tmp <- cpda::rlba(attr(data, "n.pda"), c(p.df$b, p.df$A, p.df$mean_v, p.df$sd_v,
       #   p.df$t0))
      ## cat("use rlba1\n")
       tmp <- cpda::rlba1(attr(data, "n.pda"), c(p.df$b[1], p.df$A[1],
         p.df$mean_v, p.df$sd_v, p.df$t0[1]))
       samp <- data.frame(rt=tmp[,1], response=tmp[,2])
       samp <- samp[samp$rt<10,]
           ...
           if (any(is.samp)) {
             if ( length(rts[[j]]) > 1 ) {
               ...
               likelihood[ attr(data,"cell.index")[[ cells.to.fill[j]]] ] <-
                 cpda::lik_fft(rts[[j]], samp[is.samp,"rt"], n=attr(data,"n.pda"))[,2]
             } else {
               ## cat("run lik_pw\n")
               likelihood[ attr(data,"cell.index")[[ cells.to.fill[j]]] ] <-
                 cpda::lik_pw(rts[[j]], samp[is.samp,"rt"], n=attr(data,"n.pda"))
             }
           } else {
             likelihood[ attr(data,"cell.index")[[ cells.to.fill[j]]] ] <- 0
           }
  ...
  return (pmax(likelihood,min.like))
}
...
#
# system.time({d.cpda <- exp(cpda::logLik_pw(d.c$x, rt.c, n=n))})
# system.time({d.cpda <- exp(cpda::logLik_fft2(d.c$x, rt.c, n=n)[,2])})
