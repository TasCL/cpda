...
p.vector  <- c(A=.25,mean_v.true=1,mean_v.false=.25,t0=.2)

# Note: this is the default value of n.pda
data.model <- data.model.dmc(simulate.dmc(p.vector,model,n=1e3),model,n.pda=1e4)
...
samples1 <- run.dmc(samples, report = 10, cores=12,p.migrate=.05,force=4)
plot.dmc(samples1,pll.chain=TRUE)
samples1 <- run.dmc(samples.dmc(samples=samples1,add=TRUE,nmc=100), 
  report = 10, cores=18,p.migrate=.05,force=4)
plot.dmc(samples1,pll.chain=TRUE)
plot.dmc(samples1,pll.chain=TRUE,start=200)
summary.dmc(samples1,start=200)


load_model ("LBA","lba_B.R")
Samples <- samples.dmc(nmc=400,p.prior,data.model)
Samples1 <- run.dmc(Samples, report =20, cores=18,p.migrate=.05)
plot.dmc(Samples1,pll.chain=TRUE,start=300)
summary.dmc(Samples1,start=300)
